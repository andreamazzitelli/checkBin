...
gpio_t echo_pin = GPIO_PIN(PORT_C, 7); //D9 -> echo
uint32_t echo_time;
uint32_t echo_time_start;

//load cell
gpio_t sck = GPIO_PIN(PORT_A, 6); //D12 -> SCK
gpio_t dt = GPIO_PIN(PORT_A, 7); //D11 -> DT

//stepper motor
gpio_t pin_step_1 = GPIO_PIN(PORT_B, 5); //D4 -> IN1
gpio_t pin_step_2 = GPIO_PIN(PORT_B, 4); //D5 -> IN2
gpio_t pin_step_3 = GPIO_PIN(PORT_B, 3); //D3 -> IN3
